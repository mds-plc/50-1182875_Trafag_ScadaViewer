 ...
 */
export function useFiles({ location, type, page, perPage = 50, dateFrom, dateTo }) {
    const { t } = useLang();
    const tRef = useRef(t);
    tRef.current = t;
    ...
    const fetchFiles = useCallback(async () => {
        ...
        try {
            ...
            if (!Array.isArray(json.files))
                throw new Error(tRef.current.common.errorInvalidResponse);
            setFiles(json.files);
            ...
            if (ctrl.signal.aborted)
                return; // přerušeno novějším requestem — ignorovat
...
function useDataFetch() {
    ...
    const [fileExpectedCount, setFileExpectedCount] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const abortRef = useRef(null);
    const fetchData = useCallback(async (filter) => {
        ...
        try {
            ...
            if (filter.perPage != null)
                params.set('per_page', String(filter.perPage));
...
export function useFileRecords() {
    ...
    const fetchRecords = useCallback((fileId, location, fileType, page = 1) => fetchData({
