...
const Wrapper = ({ children }) => (_jsx(LangProvider, { children: children }));
const BASE_FILTER = {
...
describe('useData', () => {
    ...
    it('sets loading true during fetch and false after completion', async () => {
        let resolveResponse;
        vi.mocked(fetch).mockReturnValueOnce(new Promise(res => { resolveResponse = res; }));
        const { result } = renderHook(() => useData(), { wrapper: Wrapper });
        ...
        // Resolvovat response
        await act(async () => {
            resolveResponse({ ok: true, json: async () => SUCCESS_RESPONSE });
        });
        expect(result.current.loading).toBe(false);
    });
